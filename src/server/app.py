...
TheOfficeApi = Flask(__name__)
cors = CORS(TheOfficeApi)
TheOfficeApi.config['CORS_HEADERS'] = 'Content-Type'
...
@cross_origin()
@TheOfficeApi.route('/', methods=['GET'])
def dashboard():
    return jsonify(get_random_quote())

if __name__ == '__main__':
    TheOfficeApi.run(host= '127.0.0.1',port=5000)
